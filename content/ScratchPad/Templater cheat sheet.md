---
created: 2025-08-14 06:26:59
modified: 2025-08-14 06:27:38
---
...
javascript opening tag = <% closing tag = %> 
...
```ad-info There are two types of functions:
...
ad-info All function types are children of the tp object and can therefore be called with: 
...
javascript tp.'function'(arg1, arg2, arg3... [or empty]) 

Function documentation syntax

javascript tp.<my_function>(arg1_name: type, arg2_name?: type, arg3_name: type = <default_value>, arg4_name: type1|ty 
...
```ad-info The internal functions are sorted into modules. The existing modules are:
...
javascript tp.config.active_file? The active file (if existing) when launching Templater.

javascript tp.config.run_mode The RunMode, representing the way Templater was launched (Create new from template, Append to active file, ...)

javascript tp.config.target_file The TFile object representing the target file where the template will be inserted.

javascript tp.file.template_file The TFile object representing the template file.

Date module

javascript tp.date.now(format: string = "YYYY-MM-DD", offset?: number⎮string, reference?: string, reference_format?: string) Retrieves the date.
...
javascript tp.date.tomorrow(format: string = "YYYY-MM-DD") Retrieves tomorrow's date.

Arguments:

- format: Format for the date, refer to [format reference](https://momentjs.com/docs/#/displaying/format/)

javascript tp.date.weekday(format: string = "YYYY-MM-DD", weekday: number, reference?: string, reference_format?: string) 
...
javascript tp.date.yesterday(format: string = "YYYY-MM-DD") Retrieves yesterday's date.

Arguments:

- format: Format for the date, refer to [format reference](https://momentjs.com/docs/#/displaying/format/)

Moment.js Templater gives you access to the moment object, with all of its functionalities. More informations on moment.js [here](https://momentjs.com/docs/#/displaying/)

File Module

javascript tp.file.content Retrieves the file's content

javascript tp.file.create_new(template: TFile ⎮ string, filename?: string, open_new: boolean = false, folder?: TFolder) Creates a new file using a specified template or with a specified content.
...
javascript tp.file.creation_date(format: string = "YYYY-MM-DD HH:mm") Retrieves the file's creation date.

Arguments:

- format: Format for the date, refer to format reference

javascript tp.file.cursor(order?: number) Sets the cursor to this location after the template has been inserted.
...
javascript tp.file.cursor_append(content: string) Appends some content after the active cursor in the file.

Arguments:

- content: The content to append after the active cursor

javascript tp.file.exists(filename: string) Checks if a file exists or not. Returns a true / false boolean.

Arguments:

- filename: The filename of the file we want to check existence, e.g. MyFile.

javascript tp.file.folder(relative: boolean = false) 
...
javascript tp.file.include(include_link: string ⎮ TFile) Includes the file's link content. Templates in the included content will be resolved.
...
javascript tp.file.last_modified_date(format: string = "YYYY-MM-DD HH:mm") Retrieves the file's last modification date.

Arguments:

- format: Format for the date, refer to format reference.

javascript tp.file.move(new_path: string) Moves the file to the desired vault location.
...
javascript tp.file.path(relative: boolean = false) Retrieves the file's absolute path on the system.

Arguments:

- relative: If set to true, only retrieves the vault's relative path.

javascript tp.file.rename(new_title: string) 
...
javascript tp.file.selection() Retrieves the active file's text selection.

javascript tp.file.tags Retrieves the file's tags (array of string)

javascript tp.file.title Retrieves the file's title.

Frontmatter Module

javascript tp.frontmatter.<frontmatter_variable_name> Retrieves the file's frontmatter variable value.
...
javascript tp.system.clipboard() Retrieves the clipboard's content

javascript tp.system.prompt(prompt_text?: string, default_value?: string, throw_on_cancel: boolean = false) Spawns a prompt modal and returns the user's input.
...
javascript tp.system.suggester(text_items: string[] ⎮ ((item: T) => string), items: T[], throw_on_cancel: boolean = false, placeholder: string = "") Spawns a suggester prompt and returns the user's chosen item.
...
javascript tp.web.daily_quote() Retrieves and parses the daily quote from the API https://api.quotable.io

javascript tp.web.random_picture(size?: string, query?: string) Gets a random image from https://unsplash.com/
...
ad-info You can define your own functions. 
...
They can be invoked by: javascript tp.user.<user_function_name>() 

Script User Functions

ad-info Allows to call Javascript functions from file and retrieve the return value. 

First you should define your scripts folder in the settings. You will then be able to call .js files from it. For further information about Javascript click [here](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Introduction).

The function call name corresponds to the script name.

```ad-warning Scripts should follow the [CommonJS module specification](https://flaviocopes.com/commonjs/), and export a single function.
...
ad-info Allows you to execute system commands. This could be powershell, bash, cmd etc... 
...
ad-info Templater defines 3 types of opening tags, that defines 3 types of **commands**: 
...
To declare a command dynamic add a plus sign after the opening tag. Like this: javascript <%+ "command" %> 

Execution Commands

Allow to execute Javascript. They allow for global namespace variables.
...
javascript For example, the following command: `<%* tR += "test" %>` will output `test`. 

ad-warning There exist asynchronous functions in JS which. Add the await when needed. 
